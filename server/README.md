...
返答が長すぎる場合は文の途中で切らず、句点 (。！？) で区切った文を末尾から落として
120秒以内に収め直します。1文だけで超える場合は読点 (、) で区切って同じように削ります。
それでも収まらない場合はエラーにします。短縮したときは `reply-shortened` をログに出します。
VOICEVOX は非常に長い文章の合成に失敗する (500 を返す) ため、合成の失敗も「長すぎる」と同じ扱いで
短くして再試行します。一度も合成できなかった場合は、その失敗をそのままエラーとして返します。
