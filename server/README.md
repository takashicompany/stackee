...
`sample_rate`、`channels`、`sample_width` が入ります。字幕があるときは `subtitles_url` も
入ります。1秒間隔のポーリングを想定します。
