...
## 6a3. マイクを開ける費用（2026-09-21）

押してから使える音が録れ始めるまで約 140 ms かかっていた。**まず測る口を足し**
（`audio.selftest {"onset":1}`: `open_ms` / `i2c_us` / `first_ms` / `csm_ms` /
`head_rms[10]`）、実測してから直した。

- **測ってから直す。** 見立て（研究メモ）と実測が一致したのは `csm_ms`（96 ms の
  計算に対して実測 99〜101 ms）と `i2c` の 40 ms 台。数字が出てから初めて
  「どこを削るか」を決められる。
- **A1: 同じ初期化を二度打ちしない。** `es7210_mic_select()` が `setup()` と
  その中の `enable(true)` で 2 回走っていた。
- **A2: 全レジスタ設定は 1 回。** レジスタは電源を切らないかぎり残る。
  ただし**「1 回書いたから安心」にはしない** — ほかの経路が触ったかもしれない
  ときは印を立てて次で書き直し、近道に入る前に 1 往復だけ読んで値が残っているかを
  確かめ、転んだら次は必ず全設定。**古い設定のまま録らない**が約束。
  判断だけを `stackee_micopen.c`（ESP-IDF に依存しない）に出してホストで試す。
- **★ 削ってはいけないものがあった。** A2 で全設定を飛ばすと ES7210 の初期化期間
  （CSM の 96 ms）も飛ぶ。その 96 ms が**アナログを上げ直したときの突入を隠して
  いた**ので、飛ばした版は録音の頭 50 ms が 140 → 21,800 RMS になった。
  近道でも CSM だけは明示的に走らせ直す。**速くすることと壊さないことは別の判断。**
- **A6/A7（96 ms 自体を削る・順序を変える）はやらない。** 下限がデータシートに
  無い、初期化順は過去に踏んだ場所。効果は大きいが、根治は別の道
  （押す前の音を持っておくプリロール）にある。
