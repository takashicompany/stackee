...
## 6a4. 案内の字幕（2026-09-22）

帯が黒いだけでは「いま何をすればいいか」が伝わらない。会話の状態をそのまま
言葉にして帯へ出す（録音中 =「マイクに向かって話しかけてください」、
考え中 =「考えています…」）。

- **行の割り方はサーバと同じ規則。** 返答の字幕と切れ目の作法をそろえないと、
  同じ帯に出ているのに見え方だけ違うものが混ざる。
- **★ 帯の持ち主は常に 1 人。** 一次回答（audio が出す）、返答の字幕（会話の
  状態機械が出す）、案内（同）が同じ 240x70 を取り合う。持ち主が居るときは
  案内は**書きも消しもしない**（`GUIDE_HELD`）。消す責任は出した側にある、
  という形にしておかないと「消したつもりが相手のものを消していた」が起きる。
- **嘘を出したままにしない。** 返答に字幕が無ければ、鳴らし始めに案内を消す
  （「考えています…」のまま鳴り続けない）。短押し・無音で捨てたときも消す。
- **文面は定数で、差し替えられる形にしておく**（`stackee_talk_set_guides()`）。
  いまは settings から読んでいない — 必要になってから繋ぐ。
...
## 6c2. 書き込みをページ経由で確かめる（2026-09-22）

`tools/ota.mjs` は操作盤と同じ中核 JS を import しているが、**ページそのものは
通らない**。ボタンの有効・無効、ファイル選択、確認ダイアログ、進捗の出し方、
再起動後の表示 — 人が見るものは 1 つも確かめられていなかった。
`tools/ota_browser.mjs` が**本物の Chrome でページを開き、人と同じ順番で
クリックする**。

- **ヘッドレス。** ウィンドウを出さずフォーカスも奪わない。ユーザーの作業を
  止めないのが条件。Playwright の既定のヘッドレスは機能を削った
  `chromium_headless_shell` なので、`channel: 'chromium'` で「新しいヘッドレス」
  を使う。
- **★ 機器選択ダイアログは自動化できない**（研究メモ §3）ので、**使い捨て
  プロファイルに「許可済み」と書いてから開く**。Chrome が許可を残す形
  （`hid_chooser_data`）をそのまま書く。シリアル番号を持つ機器だけこの形で
  永続し、stackee は持っている。**root も管理ポリシーも要らず、後に何も
  残らない。** 管理ポリシー（`WebHidAllowDevicesForUrls`）はユーザー領域に
  書いても効かないことを実機で確かめた（`chrome://policy` に出ない）。
  効かせるには `/Library/Managed Preferences/` = root。
- **既定は手元の `docs/` を 127.0.0.1 で配る。** 未 push のページをそのまま
  試せる。公開 URL も `--page` で選べる。
- **ページの文面を読んで終わる。** 完了判定はページが出す日本語そのもの
  （「書き込んで切り替えました (N 秒)。いまは … で動いています。」）。
  ホストテストが**ページ側の文面と読み取り側の正規表現を突き合わせる**ので、
  文面を変えたら気づく。
