...
#   空いた 66 px を回した)。塗るのは 7,200 -> 23,040 画素 (14.4 -> 46 KB)。
#   線形に伸ばすと 2.4 ms のはずが、実機は **空の帯だけで 3.2 ms** かかる。
#   理由はフレームバッファが PSRAM にあり、**データキャッシュが 32 KB**
#   (CONFIG_ESP32S3_DATA_CACHE_32KB) だから: 14.4 KB の帯はキャッシュに
#   丸ごと載っていたが、46 KB は載らないので描き直すたびに PSRAM まで往復する。
#   実測 (f2c6688、顔を止めて Wi-Fi up、40 回の最小値):
#     空 3,185 us / 1 行 15 桁 4,061 us / 4 行 4,594 us / **4 行 15 桁 6,188 us**
#   8 ms は「いちばん重い中身 (4 行 x 15 桁 = 全角 60 字) に 1.3 倍の余裕」。
#   ★ 帯を描くのは ui タスク (優先度 3) で、しかもページが変わった時だけ。
#     打鍵は CPU1 の最高優先度なので 1 ミリ秒も響かない (下の「字幕中」で実測)。
SUB_PAINT_MAX_US = 8000
