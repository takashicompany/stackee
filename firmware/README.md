...
| 約束 | 実測 |
|---|---|
| 帯 1 回の**描画そのもの**は **≤ 2 ms** (4 行にしたあとは ≤ 8 ms。§23-8) | 空 **164 us** / 半角混在 442 us / 全角 15 桁 **631 us** / はみ出し 16 桁 714 us。240x30 = 7,200 画素 (14.4 KB) の塗りと、多くても 15 字の点打ちだけ |
...
(1 行・2 行・4 行・4 行 15 桁・頁めくり直後・5 行目は捨てる・半角混在・
...
**線形に伸ばした見積もり (2.4 ms) では足りなかった。** 空の帯を塗るだけで
**3.2 ms** かかる。理由は**データキャッシュが 32 KB**
(`CONFIG_ESP32S3_DATA_CACHE_32KB`) で、フレームバッファが **PSRAM**
(`stackee_lcd.c` は `MALLOC_CAP_SPIRAM` で取る) だから:

* 1 行の帯 240x30 = **14.4 KB** はキャッシュに丸ごと載る。同じ帯を続けて
  描き直すと PSRAM まで行かないので 164 us で済んでいた。
* 4 行の帯 240x96 = **46 KB** は載らない。描き直すたびに全面が PSRAM との
  往復になる (書き込みは read-allocate なので、バスを通るのは約 92 KB)。

実測 (`f2c6688`、顔を止めて Wi-Fi up、40 回の**最小値** = 割り込まれなかった標本):
...
顔を自動 (アニメーション中) にしても最小値は動かない (空 3,172 us /
4 行 4,584 us)。**描画そのものの費用**で、割り込みではない。

そこで `check_phase2.py` の `SUB_PAINT_MAX_US` を **8,000 us** にし、
いちばん重い中身 (4 行 x 15 桁) を検査のケースに足した (12 → 13 ケース)。
8 ms は最悪値に 1.3 倍の余裕。

★ **これで困らない理由。** 帯を描くのは **ui タスク (優先度 3)** で、しかも
**ページが変わった時だけ** (1 往復で 3〜4 回)。打鍵は CPU1 の最高優先度の
入力タスクが扱うので、字幕を描いている最中の実測は
**中央値 1.615 ms / 最大 1.846 ms** (合否 6 / 10 ms) と平常時と変わらない。
同じ ui タスクが描くステータスバー (中央値 1,641 us / 最大 75,355 us) や
LCD 転送 (最大 193,140 us) と比べても軽い。

★ 減らしたいなら道はある (どれも今回は**やっていない**):
「変わった行だけ描き直す」(本体が行の状態を持つことになり、`ui.subtitle`
一発で同じ絵を作れなくなる = CRC 照合が使えなくなる)、
「帯を内蔵 RAM に持って転送時だけ写す」(46 KB の常駐が増える)。

#### 実機での確認 (`f2c6688`、full、2026-09-21)
...
[OK] 打鍵の遅延 (平常)   中央値 1.489 ms / 最大 1.648 ms / 12 回
[OK] 打鍵の遅延 (描画中)  中央値 0.813 ms / 最大 1.710 ms / 7 回
[OK] 打鍵の遅延 (字幕中)  中央値 1.615 ms / 最大 1.846 ms / 13 回
```

無音 (`audio.null`) の往復 6 回で、帯に行が 1 本ずつ積まれることを実機で確認
した。帯の CRC32 はどれも `subtitle_expected.py` の期待値と一致する
(下の「本文」は推測ではなく **CRC が合った文字列**)。

| 往復 | 返答 | ページ | 帯の CRC32 | 積まれた行 | `ui.status` の `sub_len` |
|---|---|---|---|---|---|
| 2 | 「はい、了解です。ほかに調べたいことがあれば、いつでも声をかけてください。」 | 0 | 1520847335 | 1 行 「はい、」 | 9 B |
| | | 1 | 2695004188 | 2 行 (+「了解です。」) | 25 B |
| | | 2 | 3383337185 | 3 行 (+「ほかに調べたいことがあれば、」) | 68 B |
| | | 3 | 1359594250 | **4 行** (+「いつでも声をかけてください。」) | 111 B |

★ **頁めくり (5 行目) は会話では出せなかった。** 6 往復ぜんぶ返答が
3〜4 ページで終わった (いちばん長いもので 4 ページ = ちょうど 4 行)。
代わりに、9 ページぶんの帯の文字列を**ホストビルドの `stackee_talk_band` に
出させて**そのまま実機の `ui.subtitle` へ投げ、9/9 とも期待値と一致することを
確かめた:

```
頁 0 → 1 行 'ぺ0'                    crc 3114546774
頁 3 → 4 行 'ぺ0\nぺ1\nぺ2\nぺ3'      crc 3185715274
頁 4 → 1 行 'ぺ4'                    crc  946265165   ← 頁めくり
頁 7 → 4 行 'ぺ4\nぺ5\nぺ6\nぺ7'      crc  339259010
頁 8 → 1 行 'ぺ8'                    crc   27198827   ← 2 回目の頁めくり
```

**まだ確かめていないこと**: 5 ページ以上の返答を**サーバから受けて**
状態機械が頁をめくるところ。サーバの返答の長さはこちらから決められない
(`talk.inject` は決まった PCM を送るだけ) ので、長い返答が来る往復を
待つしかない。行の集合はページ番号だけで決まるので、上の 2 つ
(会話での 4 行 + `ui.subtitle` での 9 頁) で経路は覆えている。
