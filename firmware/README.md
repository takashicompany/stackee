...
通信は**専用タスク** (`stackee_http`、CPU0 / 優先度 1 / スタック 10 KB) が持つ。
★ 優先度は touch・ui (3) より**下**にする (2026-09-26 に 4 → 1)。keep-alive を切って
いるので要求のたびに TLS の握手が走り、CPU0 を約 4.5〜5 秒握る (会話 1 回で 3 回)。
4 だとその間タッチと顔と console が止まり (実測: touch・ui の周回が毎秒 200 → 6〜50)、
明けたあとは touch の取り戻し周回が錠を塞いでいた (§17-1 の「遅れを取り戻さない」)。
...
#### 押してから使える音が録れ始めるまで (2026-09-21 に実測して直した)

`research/stackee/record_onset_2026-09-21.md` の見立て (約 140 ms) を実機で
確かめる口を足した。**無音・人手ゼロで測れる。**

```
python3 firmware/tools/console_hid.py  # から
# {"cmd":"audio.selftest","onset":true}
```

返すもの: `open_ms` (`enter_mic()` 全体) / `i2c_us` (そのうち ES7210 の設定) /
`first_ms` (`i2s_channel_enable` → 最初の DMA バッファ) / `csm_ms`
(同 → ES7210 の `0x0B` が `CSM_STATE=normal` になるまで) /
`head_rms[10]` (先頭 500 ms を 50 ms ずつ)。
会話の道には `talk.status` と `[talk-turn-timing]` の `first_sample_ms`
(録音を頼んでから最初のサンプルが手に入るまで) が出る。

**実測 (5 回ずつの中央値)**

| | 前 | 後 (A1+A2) |
|---|---:|---:|
| `open_ms` … 押下の道でそのまま待たされる | **44 ms** | **6 ms** |
| `i2c_us` … そのうち ES7210 の I2C | **44,213 us** | **6,078 us** |
| `first_ms` … enable → 最初の DMA | 19 ms | 19 ms (変わらない) |
| `csm_ms` … enable → CSM が `normal` | 100 ms | 100 ms (変わらない) |
| `head_rms` 先頭 500 ms (50 ms ずつ) | `0 / 140 / 2415 / 371 / 90 / …` | `0 / 238 / 2565 / 315 / 69 / …` |

★ **`csm_ms` = 100 ms はデータシートの式どおり。** `0x09`/`0x0A` に
書いている `0x30` (=48) を `period = LGTH / LRCK * 16` に入れると
16 kHz で 48 ms x 2 状態 = 96 ms。**実測 99〜101 ms。** しかも
`csm_before` は毎回 `1` (chip initial) — LRCK が出て初めて数え始める。

★ **`head_rms` の 3 枠目 (100〜150 ms) の 2,400 台がその跳ね。** 会話の
「無音なら送らない」判定が「いちばん大きい窓」では決められないのはこれが理由
(§11-4)。
...
| | 中身 | 効果 |
|---|---|---|
| **A1** | `es7210_mic_select()` の二度打ちをやめた。`setup()` の中で呼んだ直後に `enable(true)` がもう一度呼んでいた (25 往復 ≒ 9 ms)。電源を上げ直したあとに要るのは、落とすときに消した 2 つのレジスタ (`0x01` のクロックと `0x4B` のマイク電源) だけ — ゲイン (`0x43`/`0x44`) も SDP (`0x11`/`0x12`) も残っている | I2C を 25 往復ぶん |
| **A2** | **全レジスタ設定は 1 回だけ。**2 回目からは電源を上げ直すぶんだけ。ほかの経路が触ったかもしれないときは**印を立てて次で書き直す** (`stackee_es7210_invalidate()`)。近道に入る前に書いた値が残っているかを 1 往復だけ読んで確かめ、化けていたら全設定へ。**転んだら次は必ず全設定** | **44 → 6 ms** |

`audio.status` に `mic_full` (全設定した回数) / `mic_light` (電源だけで
済んだ回数) / `mic_dirty` (印が立っているか) が出る。

★★ **A2 だけでは録音の頭が壊れた。** 全設定を飛ばすと ES7210 の
**初期化期間 (CSM の 96 ms) も飛ぶ**。そしてその 96 ms が、
**アナログを上げ直したときの突入を隠していた**。飛ばした版の実測は
先頭 50 ms の RMS が **140 → 21,800**、200 ms かけて減衰する — 録音の頭に
はっきり乗るし、§11-4 の「無音なら送らない」も素通りする。
そこで近道でも **CSM を明示的に走らせ直す** (`0x00` の bit0 = `CSM_ON` を
落として上げる。I2C 2 往復)。これで頭の形は元どおりで、開ける費用だけが
44 → 6 ms になる。

★ **初回だけは全設定** (起動時ではなく最初に使うとき)。起動時に回すと
MCLK を出すことになり、**MCLK は基板の緑 LED と同じピン**なので起動の
見え方が変わる。回数は同じ 1 回なので、起動を触らないほうを選んだ。

★ `research` の **A6 (`0x09`/`0x0A` を縮める) と A7 (LRCK 先行) はやっていない。**
どちらも「録れてはいるが信用できない 96 ms」を削る話で、下限がデータシートに
無い / 初期化順を触る、というリスクがある。
...
★ **遅れは取り戻さない** (2026-09-26)。上位のタスクに数秒待たされても、過ぎた周期を
眠らずに連続で読み直すことはしない (起点を今に置き直す)。以前は vTaskDelayUntil が
遅れぶん (4.5 秒 = 約 900 周) を 2 ms ごとに回して取り戻そうとし、その間は内部 I2C の錠を
放した直後にまた取るので、優先度の低い使い手 (console の `aldo3`・電池、camera タスクの
ALDO3 入り切り) が 200 ms 待っても錠を取れなかった。`touch.status` の `late` が
「周期に遅れた回数」。
