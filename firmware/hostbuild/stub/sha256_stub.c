...
// FIPS 180-4 のそのまま。速さは要らない (テストで数 MB 流すだけ)。
...
static void block(sha256_stub_t *ctx, const uint8_t *p) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
...
void sha256_stub_update(sha256_stub_t *ctx, const void *data, size_t len) {
    ...
    while (len > 0) {
        size_t take = 64 - ctx->len;
        if (take > len) {
            take = len;
        }
        memcpy(ctx->buf + ctx->len, p, take);
        ctx->len += take;
        p += take;
        len -= take;
        if (ctx->len == 64) {
...
void sha256_stub_final(sha256_stub_t *ctx, uint8_t out[32]) {
    ...
    uint8_t pad = 0x80;
    sha256_stub_update(ctx, &pad, 1);
    uint8_t zero = 0;
    while (ctx->len != 56) {
        sha256_stub_update(ctx, &zero, 1);
    }
    uint8_t tail[8];
    for (int i = 0; i < 8; i++) {
        tail[i] = (uint8_t)(bits >> (56 - i * 8));
    }
    sha256_stub_update(ctx, tail, 8);
    for (int i = 0; i < 8; i++) {
