...
## マイクを開ける費用を測って減らす 2026-09-21

像 `482ba08`(full、1,396,624 B、名札 `f332d509…`)。
出発点は `research/stackee/record_onset_2026-09-21.md`(見立て 約 140 ms)。

### 段 1: 測る口を足した(`audio.selftest {"onset":1}`)

**無音・人手ゼロ。**マイクを開けて 500 ms 録るだけで、次を返す:
`open_ms`(`enter_mic()` 全体)/ `i2c_us`(そのうち ES7210 の設定)/
`first_ms`(`i2s_channel_enable` → 最初の DMA バッファ)/
`csm_ms`(同 → `0x0B` の `CSM_STATE` が normal)/ `head_rms[10]`(先頭 500 ms を 50 ms ずつ)。
会話の道には `first_sample_ms` を足した。

### 段 2 の前(押すたび全設定)— 5 回

| 回 | `open_ms` | `i2c_us` | `enable_us` | `first_ms` | `csm_ms` | `csm_before` |
|---|---:|---:|---:|---:|---:|---:|
| 1 | 52 | 45,120 | 16,296 | 19 | 100 | 1 |
| 2 | 43 | 43,735 | 15,092 | 19 | 99 | 1 |
| 3 | 44 | 44,213 | 15,333 | 19 | 100 | 1 |
| 4 | 44 | 44,055 | 15,359 | 19 | 100 | 1 |
| 5 | 44 | 44,690 | 15,230 | 19 | 101 | 1 |
| **中央値** | **44** | **44,213** | **15,333** | **19** | **100** | 1 |

`head_rms`(50 ms x 10): `0 / 140 / 2415 / 371 / 90 / 91 / 74 / 102 / 100 / 0`

★ **見立てどおりだった。** `csm_ms` はデータシートの式(`0x09`/`0x0A` = 0x30 →
16 kHz で 48 ms x 2 = **96 ms**)に対して実測 **99〜101 ms**。`csm_before` は
毎回 1 (chip initial) で、**LRCK が出て初めて数え始める**ことも裏が取れた。
`i2c_us` も見立て 40 ms に対して 44 ms。`first_ms` 19 ms は DMA の
240 frame = 15 ms + 汲み上げ。

★ **`head_rms` の 3 枠目 (100〜150 ms) の 2,400 台が立ち上がりの跳ね。**
CSM が normal になる時刻とぴたり合う。会話の「無音なら送らない」で
「いちばん大きい窓」が使えない理由がこれ(同じ日に別途実装。§11-4)。
...
|---|---:|---:|
| `open_ms` | **44 ms** | **6 ms** |
| `i2c_us` | **44,213 us** | **6,078 us** |
| `enable_us` | 15,333 us | 6,070 us |
| `first_ms` | 19 ms | 19 ms |
| `csm_ms` | 100 ms | 100 ms |
| `head_rms` | `0/140/2415/371/90/…` | `0/238/2565/315/69/…` |

`audio.status`: `mic_full=1 / mic_light=5 / mic_dirty=false`
(全設定は 1 回きり、あとは電源を上げ直すだけ)。

### ★ A2 だけでは録音の頭が壊れた(途中で見つけて直した)

全設定を飛ばすと **ES7210 の初期化期間(CSM の 96 ms)も飛ぶ**。
そしてその 96 ms が**アナログを上げ直したときの突入を隠していた**。
飛ばしたままの版の実測:

| | 0-50 | 50-100 | 100-150 | 150-200 | 200-250 |
|---|---:|---:|---:|---:|---:|
| 押すたび全設定 | 0 | 140 | 2,415 | 371 | 90 |
| **A2 だけ(CSM も飛ぶ)** | **21,825** | **9,263** | 4,005 | 262 | 92 |
| A1+A2 + CSM を走らせ直す | 0 | 238 | 2,565 | 315 | 69 |

`csm_before` も 2 (normal のまま) になっていて、**状態機械が一度も初期化を
やり直していなかった**。録音の頭に -3.5 dBFS 級の突入が乗るうえ、
§11-4 の「無音なら送らない」判定(1000 を 5 窓)も素通りしてしまう。

→ 近道でも **`0x00` の bit0 = `CSM_ON` を落として上げる**(I2C 2 往復)。
これで頭の形は元どおり、開ける費用だけが 44 → 6 ms になった。

**速くすることと壊さないことは別の判断**、という例。

### check_phase1 / 2 / 3

* `check_phase1` 全項目 OK。
* `check_phase2` 全項目 OK。
* `check_phase3`(`--allow-sound` なし = 無音)は **内蔵 RAM 以外すべて OK**:
  マイク 15,872 サンプル / RMS 580、スピーカー 53,077 サンプルを 3,319 ms、
  Wi-Fi up、**会話 1 往復 成功**(返答「うん、了解なのだ。」/ 返答 68,950 B)、
  打鍵の遅延 会話中 中央値 1.50 ms / 最大 1.99 ms、音量キー 10→15% と
  ステータスバーの絵・NVS 保存、BLE。
  * `[NG] 内蔵 RAM 空き 19,307 B`(合否 40,000 B)は **今回の変更より前から**。
    研究メモが `c6e0181` 時点で読んだ実機の値が既に `heap_internal 21283`
    (README §24 の既知の細さ)。今回で `.dram0.bss` は 75,208 → **76,328 B
    (+1,120 B。計測の入れ物と印)**。

本体は元の状態に戻した(`audio.null` false、音量 10%、顔・バー・帯は自動)。

### ホストテスト

**487 件**(前 479 件)。`test_micopen_host.py`(8 件、新規)が
「1 回目は全設定・2 回目からは電源だけ」「印を立てたら次の 1 回だけ全設定」
「書いた値が残っていなければ全設定」「転んだら次は必ず全設定」を見る。
判断だけを `main/stackee_micopen.c`(ESP-IDF に依存しない)へ出してある。

### 未検証のまま

| 項目 | なぜ |
|---|---|
| **A6 / A7** | 指示どおりやっていない。CSM の 96 ms 自体は残っている |
