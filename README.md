...
- アプリ内 OTA の中核 (`test/ota.test.mjs`): 0xC3 の枠の組み立てと読み取り、
...
- アプリ内 OTA の Node 経路 (`test/ota_node.test.mjs`): `firmware/tools/ota.mjs` の
  転送層を**偽のデバイス**で回す。Report ID を足した 33 バイトで書くこと、
  0xC0 に 29 バイトずつ詰めること、0xC1 の取り出しを転送中は止めること、
  0xC3 の応答が credit に回ること、像が 1 バイトも欠けず・重複せず届くこと。
  **node-hid も実機も要りません** (`ota.mjs` は `openDevice()` の中で初めて
  node-hid を読むため)
